...
int main()
{
    ...
    int iterations_per_process = total_iterations / world_size;
    ...
    for (int i = 0; i < N; i++) {
        if ((inner_iterations >= world_rank * iterations_per_process) && start_index == -1) {
            start_index = i;
        }
        if (inner_iterations >= (world_rank+1) * iterations_per_process) {
            end_index = i;
            break;
        }
...
double computeAccelerationsAndPotential() {
    ...
    for (int i = start_index; i < end_index; i++) {   // loop over all distinct pairs i,j
    ...
    // reduce the Potential value into the root process
    MPI_Reduce(&Pot, &total_potential, 1, MPI_DOUBLE, MPI_SUM, root_rank, MPI_COMM_WORLD);

    //reduce and broadcast to all processes at the same time
    // We use MPI_IN_PLACE to avoid creating local arrays and having to copy them
    MPI_Allreduce(MPI_IN_PLACE, acceleration.x, N, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(MPI_IN_PLACE, acceleration.y, N, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(MPI_IN_PLACE, acceleration.z, N, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
